...
export default class GameScene extends Phaser.Scene {
  ...
  spawnCharactersAndCoins() {
    const groundY = GAME_H - 210;
    const leftX = 240;
    const rightX = GAME_W - 240;

    // P1
    const p1C = this.add.container(leftX, groundY);
    const p1Sprite = this.add.sprite(0, 0, 'char_p1').setOrigin(0.5, 1);
    p1C.add(p1Sprite);
    this.tweens.add({ targets: p1C, y: groundY - 4, duration: 1200, yoyo: true, repeat: -1, ease: 'sine.inOut' });
    this.p1.charContainer = p1C;
    this.p1.hpBar = this.createHPBar(leftX - 110, groundY - 160);
    ...
    const p2Sprite = this.add.sprite(0, 0, 'char_p2').setOrigin(0.5, 1);
    p2C.add(p2Sprite);
    ...
    this.p2.hpBar = this.createHPBar(rightX - 110, groundY - 160);
    ...
    // Names above HP bars
    this.add.text(leftX, groundY - 180, this.p1.name, { fontSize: '14px', color: '#4aa3df', fontStyle: 'bold' }).setOrigin(0.5);
    this.add.text(rightX, groundY - 180, this.p2.name, { fontSize: '14px', color: '#df9a4a', fontStyle: 'bold' }).setOrigin(0.5);
  }
  ...
  makeButton(x, y, label, onClick) {
    ...
    const text = this.add.text(90, 22, label, { fontSize: '13px', color: '#fff' }).setOrigin(0.5);
  ...
  startTurn() {
    ...
    this.turnTimer = this.time.addEvent({
      delay: 1000, repeat: 2, callback: () => {
        this.timeLeft--;
        this.updateTimerText();
        if (this.timeLeft <= 0) this.time.delayedCall(100, () => this.finalizeSelections());
      }
    });
  }
  ...
  resetGame() {
    ...
    // Keep names, reset stats
    this.p1 = Object.assign(this.p1, makePlayer(this.p1.name));
    this.p2 = Object.assign(this.p2, makePlayer(this.p2.name));
